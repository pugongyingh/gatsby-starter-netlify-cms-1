...
class Blog extends React.Component<P> {
	public render() {
		const blogPosts =
			...
			this.props.blogPosts.filter((post) => post.frontmatter!.locale === this.props.locale);
		if (!blogPosts.length) {
			return null;
		}
		return (
	...
	return (
		<StaticQuery
			...
			render={(blogPosts: Query) => (
				<StyledBlog blogPosts={blogPosts.allMarkdownRemark.edges.map((edge) => edge.node)} {...props} />
			)}
		/>
	);
};
