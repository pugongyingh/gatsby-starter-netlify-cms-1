					...
					<Row className="instagram-items-wrap">
						<Col className="instagram-item">
							<StyledCarousel dark={true}>
								{this.props.instagramPosts &&
									this.props.instagramPosts.map((post: InstaNode) => {
										return (
											<StyledInstagramItem
												key={post.id}
												src={post.original!}
...
const InstagramRoll: React.SFC<Omit<P, 'instagramPosts'>> = (props) => {
	return (
		<StaticQuery
			...
			`}
			render={(instagramPosts: Query) => (
				<StyledInstagram
					instagramPosts={instagramPosts.allInstaNode.edges.map((edge) => edge.node)}
					{...props}
				/>
			)}
		/>
